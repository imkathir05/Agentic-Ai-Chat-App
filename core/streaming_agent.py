...
from core.huggingface_agent import (
    _get_api_key as _get_hf_api_key,
    _resolve_huggingface_model,
    _build_openai_tools as _build_hf_tools,
    _to_openai_messages as _to_hf_messages,
...
def run_huggingface_agent_stream(
    messages: list[dict[str, Any]],
    api_key: str | None = None,
    model: str | None = None,
    system_prompt: str | None = None,
    ...
    yield {"type": "info", "provider": "huggingface", "model": model_name}
    ...
    for round_idx in range(django_settings.MAX_TOOL_ROUNDS):
        ...
        try:
            with httpx.Client() as http_client:
                with http_client.stream("POST", HUGGINGFACE_API_URL, headers=headers, json=body, timeout=120.0) as r:
                    if r.status_code >= 400:
                        r.read()
                        if "tool_use_failed" in r.text and tools:
                            body_no_tools = {k: v for k, v in body.items() if k not in ("tools", "tool_choice")}
                            with http_client.stream("POST", HUGGINGFACE_API_URL, headers=headers, json=body_no_tools, timeout=120.0) as r2:
                                if r2.status_code >= 400:
                                    r2.read()
                                    raise ValueError(_friendly_huggingface_error(r2.status_code, r2.text))
                                r = r2
                        else:
                            raise ValueError(_friendly_huggingface_error(r.status_code, r.text))
        ...
        if accumulated_tool_calls:
            ...
            for tc in tool_calls_list:
                ...
                call_id = tc.get("id") or f"hf-{round_idx}-{fn_name}"
...
def run_agent_stream(
    ...
    if prov == "gemini":
        ...
        yield from run_huggingface_agent_stream(
            messages,
            api_key=api_key,
            model=model,
            system_prompt=system_prompt,
